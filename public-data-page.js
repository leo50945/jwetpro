...
const performanceRows = (rows, sortField, sortDir) => rows.length ? `<div class="performance-table-scroll"><table class="public-table performance-table"><thead><tr><th>#</th>${RANKING_COLUMNS.map(column => `<th class="ranking-sortable${column.field === sortField ? ' active' : ''}" data-field="${column.field}" role="button" tabindex="0" aria-sort="${column.field === sortField ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'}">${column.label}${column.field === sortField ? `<i class="ranking-sort-arrow" aria-hidden="true">${sortDir === 'asc' ? '▲' : '▼'}</i>` : ''}</th>`).join('')}</tr></thead><tbody>${uniqueRankingAvatars(rows).map((row,index) => {const identity={...row.profile,socialPlayerId:row.id||row.profile?.socialPlayerId,displayName:row.displayName,avatarIndex:row.avatarIndex};const profileId=identity.socialPlayerId||'';return `<tr${profileId ? ` class="ranking-player-row" data-player-profile-id="${esc(profileId)}" tabindex="0" role="link" aria-label="Voir le profil de ${esc(row.displayName)}"` : ''}><td><span class="ranking-rank">${index+1}</span></td><td>${socialAvatarLink(identity,rankingAvatar(identity,row.avatarIndex))}${socialAvatarLink(identity,`<b>${esc(row.displayName)}</b>`)}</td><td><strong>${row.participationCount}</strong></td><td>${row.semifinalCount}</td><td>${row.points}</td><td><span class="level-badge">${esc(row.level)}</span></td></tr>`}).join('')}</tbody></table></div>` : '<div class="ranking-stat-empty"><strong>Aucune donnée officielle</strong><span>Ce classement apparaîtra dès que les participations, les points et les phases atteintes seront publiés.</span></div>';
...
async function initPublicPage() {
  ...
  try {
    ...
    if (mode === 'live') {
      ...
      rankingList?.addEventListener('click', event => { const th = event.target.closest('th[data-field]'); if (th) { sortByColumn(th); return; } const row = event.target.closest('tr[data-player-profile-id]'); if (row && !event.target.closest('a,button')) location.href = `./player.html?id=${encodeURIComponent(row.dataset.playerProfileId)}`; });
      rankingList?.addEventListener('keydown', event => {
        const row = event.target.closest('tr[data-player-profile-id]');
        if (row && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); location.href = `./player.html?id=${encodeURIComponent(row.dataset.playerProfileId)}`; return; }

...
const championshipIsRegistered = data => {
  const uid = activityCurrentUserId();
  if (!uid) return false;
  const fields = ['participantIds','registeredPlayerIds','playerIds','participants','registeredPlayers','registrations'];
  return fields.flatMap(field => {
    const value = data?.[field];
    return Array.isArray(value) ? value : value && typeof value === 'object' ? Object.values(value) : [];
  }).some(value => String(typeof value === 'object' ? value?.uid || value?.userId || value?.playerUid || value?.authUid || '' : value) === uid);
};
const championshipActionForUser = data => championshipIsRegistered(data) && data.status === 'registration-open'
  ? {label:'VOIR LE CHAMPIONNAT',href:progressHref(data.id),tone:'soon'}
  : championshipAction(data.status, data.id);

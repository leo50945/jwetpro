...
const profileLegacyFavorites = () => profileList(Object.prototype.hasOwnProperty.call(currentProfileData,'favoriteGames') ? currentProfileData.favoriteGames : currentProfileData.favorites);
const profileLegacyFavoriteIdentity = item => {
  if(!item || typeof item !== 'object') return '';
  const rawKind=String(item.kind||item.type||'').toLowerCase();
  const kind=/match|game|partie/.test(rawKind)?'match':/champ|tournament|tournoi/.test(rawKind)?'championship':'';
  const entityId=String(item.entityId||item.seriesId||item.matchId||item.championshipId||item.id||'').trim();
  return kind&&entityId?`${kind}:${entityId}`:'';
};
const profileLegacyFavoriteMarkup = (item,index) => `<article>${I('Star')}<div><strong>${profileEscape(profileItemLabel(item) || 'Favori')}</strong><span>${profileEscape(typeof item === 'object' ? item.type || item.kind || 'JWETPRO' : 'JWETPRO')}</span></div><button type="button" data-remove-favorite="${index}" aria-label="Retirer des favoris">${I('Trash2')}</button></article>`;
...
const renderProfileDetail = section => {
  ...
  const favorites = profileLegacyFavorites();
  const sections = {
    ...
    favorites: () => `${profileDetailHeader('Mes favoris','Retrouvez les championnats et matchs que vous aimez.')}<div class="profile-detail-list" data-profile-social-list>${favorites.length ? favorites.map(profileLegacyFavoriteMarkup).join('') : '<div class="profile-detail-empty">'+I('Star')+'<strong>Chargement des favoris…</strong></div>'}</div>`,
...
const loadProfileSocialDetail = async (section,{append=false}={}) => {
  ...
  try{
    if(section==='favorites'){
      ...
      const socialIdentities=new Set(profileSocialFavoriteDocs.map(document=>`${document.data?.kind||''}:${document.data?.entityId||''}`));
      const legacyFavorites=profileLegacyFavorites().map((item,index)=>({item,index})).filter(({item})=>{const identity=profileLegacyFavoriteIdentity(item);return !identity||!socialIdentities.has(identity)});
      const markup=profileSocialFavoriteDocs.map(profileSocialFavoriteMarkup).join('')+legacyFavorites.map(({item,index})=>profileLegacyFavoriteMarkup(item,index)).join('');
      if(markup){container.innerHTML=markup+(snapshot.size===PROFILE_SOCIAL_PAGE_SIZE?`<button class="profile-social-load-more" type="button" data-load-social-favorites>Charger plus</button>`:'');renderIcons();applyProfileLanguage();return}
      container.innerHTML=`<div class="profile-detail-empty">${I('Heart')}<strong>Aucun favori enregistré</strong><p>Aimez un match ou un championnat pour le retrouver ici.</p></div>`;applyProfileLanguage();
      return;
    }

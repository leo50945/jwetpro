...
- La couche sociale est validée par les tests unitaires et Auth/Firestore Emulator; ses fonctions, règles et index sont déployés. Le quota CPU Cloud Run de `us-central1` a imposé l’endpoint unifié de listes `listSocialRelationships` en `us-east1`; l’ancien service `listFollowing` listé dans Firebase reste inaccessible et n’est plus utilisé par l’interface. Il reste à effectuer le backfill administrateur puis la recette de production avec deux comptes réels et un joueur simulé.
